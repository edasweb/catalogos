...
for row in range(1, sheet_brand_together.max_row + 1): # To iterate over all the row and column of the sheet and get each value.
    ...
    if brand_name !=str(sheet_brand_together[f"b{row}"].value).upper().strip():
        ...
        nice_sheet.append(["Part Number", "Car Application","Car Application 2","Car Application 3","Description","Description 2","Description 3", "Retail Price Ex VAT €"
])
    ...
    if reference in references:
        temp_row = [
            reference,
            f'{sheet_brand_together[f"b{row}"].value} {sheet_brand_together[f"c{row}"].value} {sheet_brand_together[f"d{row}"].value}', 
            f'{sheet_brand_together[f"b{row}"].value} {sheet_brand_together[f"c{row}"].value} {sheet_brand_together[f"d{row}"].value} {sheet_brand_together[f"e{row}"].value}', 
            f'{sheet_brand_together[f"b{row}"].value} {sheet_brand_together[f"c{row}"].value}', 
            f'{sheet_brand_together[f"b{row}"].value} {sheet_brand_together[f"c{row}"].value} {sheet_brand_together[f"d{row}"].value} {prices.get(reference)["name"]}', 
            f'{sheet_brand_together[f"b{row}"].value} {sheet_brand_together[f"c{row}"].value} {prices.get(reference)["name"]}', 
            f'{sheet_brand_together[f"b{row}"].value} {sheet_brand_together[f"c{row}"].value} {sheet_brand_together[f"d{row}"].value} {sheet_brand_together[f"e{row}"].value} {prices.get(reference)["name"]}', 
