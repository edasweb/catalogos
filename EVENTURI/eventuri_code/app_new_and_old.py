...
for row in range(2, work_sheet.max_row + 1): # To iterate over all the row and column of the sheet and get each value.
    if(row<=156):
      all_references.append(work_sheet[f"b{row}"].value.replace(" ", ""))
      if not work_sheet[f"b{row}"].value.replace(" ", "") in onlines:
          sheet_new.append([work_sheet[f"b{row}"].value.replace(" ", "")])
          news.append(work_sheet[f"b{row}"].value.replace(" ", ""))
