...
already_in_use = {}
brand_name = None
sheet_2.append(["Manufacturer","Supplier","Price EUR","Purchase EUR","Price GBP","Purchase GBP","%","VAT","Reference","Name","EAN13","Category","Description","Meta title","Tags","Keywords","Rewrite"])
...
for row in range(3, sheet_1.max_row + 1): # To iterate over all the row and column of the sheet and get each value.
    ...
    if not sheet_1[f"d{row}"].value in no_need and already_in_use.get(sheet_1[f"d{row}"].value) !=  sheet_1[f"f{row}"].value:
      ...
      temp_row = [
          ...
          Decimal(sheet_1[f"f{row}"].value),                   # "price"
          calculate_purcharse(sheet_1[f"k{row}"].value, Decimal(sheet_1[f"f{row}"].value)), # "purchase"
          ...
          "",                   # "EAN13"
          "Home", # "category"
          slugify(sheet_1[f"e{row}"].value.upper()), # "slug"
...
wb.save("tien_all_stars_import_catalogue_new_with_gama_and_pound_filter_price.xlsx")
