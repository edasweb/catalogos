...
for row in range(1, sheet_1.max_row + 1): # To iterate over all the row and column of the sheet and get each value.
    ...
    if brand_name != sheet_1[f"a{row}"].value:
        ...
        sheet_2.append(["Part Number", "Model", "Chassis", "Item","Note","Year","Drive_system","Displacement","Range","Edfc","Ft","Rr","Ft","Rr","Max High","Max Low","Max High","Max Low","Matching Remarks", "Retail Price Ex VAT",
    ...
    temp_row = [
        ...
        sheet_1[f"a{row}"].value + " " + str(sheet_1[f"b{row}"].value) + " " + str(sheet_1[f"b{row}"].value), 
