...
sheet_name_on_website = "on_website"
# print(wb.sheetnames) 
...
for row in range(1, sheet_on_web_site.max_row + 1): # To iterate over all the row and column of the sheet and get each value.
    ...
    reference = sheet_on_web_site[f"c{row}"].value if sheet_on_web_site[f"c{row}"].value != None else None,
...
wb.save("p3_only_news_all_stars_import_catalogue_new.xlsx")
