...
for row in range(2, sheet_1.max_row + 1): # To iterate over all the row and column of the sheet and get each value.
    ...
    temp_row = [
        ...
        sheet_1[f"b{row}"].value + " " + sheet_1[f"c{row}"].value.replace("model", "")  + getString(sheet_1[f"d{row}"].value)+ " "+ getString(sheet_1[f"l{row}"].value),  # "name"
        "Home",                   # "category"
         sheet_1[f"b{row}"].value + " " + sheet_1[f"c{row}"].value.replace("model", "")  + getString(sheet_1[f"d{row}"].value)+ " "+ getString(sheet_1[f"l{row}"].value),  # "Meta title"
...
wb.save("scorpion_all_stars_import_catalogue_new_reduce.xlsx")
